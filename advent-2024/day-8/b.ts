/* --- Day 8: Resonant Collinearity --- Part B --- */
// https://adventofcode.com/2024/day/8#part2

const file = Bun.file('input.txt');
const input = await file.text();

const testInput = `............
........0...
.....0......
.......0....
....0.......
......A.....
............
............
........A...
.........A..
............
............`;

function parseInput(input: string) {
  return input.split('\n').map((row) => row.split(''));
}

function findAntennas(grid: string[][]): Map<string, number[][]> {
  const antennas = new Map<string, number[][]>();

  for (let y = 0; y < grid.length; y++) {
    const row = grid[y];
    for (let x = 0; x < row.length; x++) {
      const cell = row[x];
      if (cell !== '.') {
        const coords = antennas.get(cell);
        if (coords) {
          coords.push([x, y]);
        } else {
          antennas.set(cell, [[x, y]]);
        }
      }
    }
  }

  return antennas;
}

function areCollinear(
  point: number[],
  antenna1: number[],
  antenna2: number[]
): boolean {
  return (
    (point[0] - antenna1[0]) * (antenna2[1] - antenna1[1]) ===
    (point[1] - antenna1[1]) * (antenna2[0] - antenna1[0])
  );
}

function findUniqueAntinodes(input: string): number {
  const grid = parseInput(input);
  const antennasMap = findAntennas(grid);
  const antinodes = new Set<string>();

  for (const antennas of antennasMap.values()) {
    if (antennas.length < 2) {
      continue;
    }

    for (let y = 0; y < grid.length; y++) {
      const row = grid[y];
      for (let x = 0; x < row.length; x++) {
        for (let i = 0; i < antennas.length; i++) {
          let collinear = false;
          const antenna1 = antennas[i];
          for (let j = i + 1; j < antennas.length; j++) {
            const antenna2 = antennas[j];
            if (areCollinear([x, y], antenna1, antenna2)) {
              antinodes.add(`${x},${y}`);
              collinear = true;
              break;
            }
          }

          if (collinear) {
            break;
          }
        }
      }
    }
  }

  return antinodes.size;
}

console.log('Unique antinodes: ', findUniqueAntinodes(input)); // Unique antinodes: 1352
