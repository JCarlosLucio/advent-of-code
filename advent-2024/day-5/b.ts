...
function fixOrder(page: number[], rules: Rules): number[] {
  for (let i = page.length; i > 0; i--) {
    for (let j = 0; j < i - 1; j++) {
      const isCorrectOrder = rules.get(page[j])?.has(page[j + 1]);
      if (!isCorrectOrder) {
        [page[j], page[j + 1]] = [page[j + 1], page[j]]; // swap values
      }
    }
  }
  return page;
...
function sumMiddlePages(input: string): number {
  ...
  for (const page of pages) {
    if (!checkOrder(page, rules)) {
      const middle = getMiddlePage(fixOrder(page, rules));
      sum += middle;
    }
  }
...
console.log('Sum of fixed middle pages: ', sumMiddlePages(input)); // Sum of fixed middle pages: 5448
