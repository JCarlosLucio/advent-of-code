...
for (const instruction of instructions) {
  ...
  const removed = stacks[from].splice(stacks[from].length - moves, moves);
  stacks[to] = stacks[to].concat(removed);
}

const topOfStacks = stacks.map((stack) => stack.at(-1)).join('');

console.log('Top of stacks: ', topOfStacks); // Top of stacks: BNTZFPMMW
